document.addEventListener('DOMContentLoaded', function() {
    ...
    async function scoreProject(projectId) {
        ...
        document.getElementById('score-project').addEventListener('submit', async function(event) {
            event.preventDefault();
            const scores = Array.from(document.querySelectorAll('input[type="radio"]:checked')).map(input => parseInt(input.value));
            const totalScore = scores.reduce((acc, score) => acc + score, 0);
            const averageScore = totalScore / scores.length;

            console.log('Submitting score for project', { projectId, averageScore });
